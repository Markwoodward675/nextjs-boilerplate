...
export default function AlertsPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (!cancelled) {
          setError(err?.message || "Unable to load alerts. Please try again shortly.");
        }
      } finally {
  ...
  const handleClaimBonus = async (alert) => {
    ...
    try {
      ...
      setClaimMessage(
        `Signup bonus claimed successfully. New balance: $${Number(res?.newBalance || 0).toLocaleString(
          undefined,
        ...
        <div className="mx-auto max-w-4xl space-y-4">
          <header>
            ...
            <p className="text-sm text-slate-400">
              Platform notifications, educational tips, and your $100 signup bonus all live here.
            </p>
          </header>
          ...
          <section className="space-y-2">
            {sorted.length === 0 ? (
              <p className="text-sm text-slate-400">
                You have no alerts yet. Once you start using the platform, updates will appear here.
              </p>
            ) : (
              sorted.map((alert) => {
                const isSignupBonus = alert.category === "signup_bonus";
                const isClaimed = alert.claimed || alert.status === "claimed";
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-2 flex items-center justify-between text-[11px] text-slate-500">
                      <span>Status: {alert.status || (alert.claimed ? "claimed" : "active")}</span>
