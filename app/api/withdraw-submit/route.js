// app/api/withdraw-submit/route.js
...
export async function POST(req) {
  try {
    ...
    const { userId, walletId, amount, currencyType = "USD", method = "bank", details = {} } = body || {};
    ...
    const tx = await db.createDocument(DATABASE_ID, COL_TX, ID.unique(), {
      ...
      status: "pending",
      meta: JSON.stringify({ method, details }),
      type: "withdraw",
    });
    ...
    await db.createDocument(DATABASE_ID, COL_ALERTS, ID.unique(), {
      ...
      alertMessage: `Withdrawal request submitted (${method}).`,
      severity: "info",
      userId,
      isResolved: false,
      title: "Withdrawal submitted",
      body: `Withdrawal request submitted (${method}).`,
