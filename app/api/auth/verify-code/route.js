...
export async function POST(req) {
  try {
    ...
    // ensure user exists (prevents verifying random ids)
    ...
    // Mark verified on profile doc (docId = userId)
    try {
      await db.updateDocument(DATABASE_ID, PROFILE_COL, userId, {
        verificationCodeVerified: true,
        verifiedAt: now,
      });
    } catch {
      await db.createDocument(DATABASE_ID, PROFILE_COL, userId, {
        userId,
        verificationCodeVerified: true,
        verifiedAt: now,
        createdAt: now,
