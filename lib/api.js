...
async function ensureWallets(userId) {
  ...
  // 36-char UUID for walletId (matches size: 36)
  const uuid36 = () => {
  ...
  const makeWallet = (currencyType) => {
    ...
    return {
      walletId,
      userId,
      currencyType,     // enum: main / trading / affiliate
      balance: 0,        // double >= 0
      isActive: true,    // boolean
      createdDate: nowIso, // datetime
      updatedDate: nowIso, // datetime
    };
  };
...
// Optional email-link verification (secondary flow)
export async function resendVerificationEmail(redirectUrl) {
  // Appwrite sends an email-link verification
  // redirectUrl should be your production URL + "/verify" (or any page)
  const url =
    redirectUrl ||
  ...
  try {
    // Appwrite Web SDK supports this
