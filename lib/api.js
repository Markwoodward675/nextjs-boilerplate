...
async function ensureWallets(userId) {
  ...
  const makeWallet = (type) => {
    const walletId = IDHelper.unique(); // ✅ required field + unique value

    return {
      walletId,          // ✅ REQUIRED by your schema
      userId,            // string
      type,              // main / trading / affiliate (must exist in schema)
      balance: 0,
      currency: "USD",
      status: "active",
  ...
  // Use docId = walletId so it's consistent + easy to getDocument later
