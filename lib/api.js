...
export async function getUserProfile(userId) {
  ensureDbConfigured();
  try {
    return await databases.getDocument(DB_ID, COLLECTIONS.user_profile, userId);
  } catch {
    return null;
  }
}
...
const fallbackSuffix = String(Math.floor(1000 + Math.random() * 9000));
const username = (emailPrefix || baseName || "user") + "-" + fallbackSuffix;

const data = {
  userId,
  username, // ✅ required attribute in your schema
