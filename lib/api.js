...
/* -------------------------------------------------------------------------- */
/* Collections (ALWAYS have fallbacks to avoid “collectionId missing”)         */
/* -------------------------------------------------------------------------- */
...
export async function registerUser({ fullName, email, password }) {
  try {
    ...
    // best-effort bootstrap (won’t work until DB permissions are correct)
    try {
    ...
    throw new Error(getErrorMessage(err, "Sign up failed. Please try again."));
  }
}

export async function loginWithEmailPassword(email, password) {
  try {
    // Clear conflicting session (optional)
    try {
...
/* -------------------------------------------------------------------------- */
/* Bootstrap (Profile doc ID MUST be user.$id)                                 */
/* -------------------------------------------------------------------------- */
...
async function createUserProfileIfMissing(user) {
  ...
  // ✅ If your schema requires "username", we provide it
  const profileData = {
    userId,
...
/**
 * Call after signup AND after login.
 * Creates profile (docId=userId), wallets, signup bonus alert.
 */
...
export async function confirmVerificationCode(inputCode) {
  ...
  const expiresAt = String(profile?.verificationCodeExpiresAt || "");
...
/* -------------------------------------------------------------------------- */
/* Signup bonus claim (exported cleanly; NO conflict markers)                  */
/* -------------------------------------------------------------------------- */
