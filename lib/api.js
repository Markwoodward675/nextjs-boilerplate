...
export async function updateUserProfile(userId, patch) {
  ...
  const filtered = safePatch(existing, {
    ...patch,
    updatedAt: nowISO(),
  });
...
// Add these collection names if you use env, otherwise keep literal IDs:
const COLLECTIONS = {
...
// Withdraw: write a transaction row
export async function createWithdrawalRequest(userId, payload) {
  ensureDbConfigured();
  if (!userId) throw new Error("Missing userId");
  ...
  return await databases.createDocument(DB_ID, COLLECTIONS.transactions, IDHelper.unique(), data, perms);
}

// Affiliate: ensure affiliate_account exists
export async function ensureAffiliateAccount(userId) {
  ensureDbConfigured();
  if (!userId) throw new Error("Missing userId");

  // IMPORTANT: Your affiliate_account schema currently has userId as integer.
  // Recommended: change affiliate_account.userId to STRING in Appwrite.
  const res = await databases.listDocuments(DB_ID, COLLECTIONS.affiliateAccount, [
    QueryHelper.equal("userId", userId),
    QueryHelper.limit(1),
  ]);
  ...
  const now = nowISO();
  const affiliateId = Math.floor(100000 + Math.random() * 900000); // 6-digit id

  const data = {
    affiliateId,         // integer
    userId,              // string (requires schema change)
    commissionRate: 10,  // example %
    totalEarned: 0,
    joinDate: now,
    status: "active",
  };
  ...
  return await databases.createDocument(DB_ID, COLLECTIONS.affiliateAccount, IDHelper.unique(), data, perms);
}
...
export async function getAffiliateSummary(userId) {
  ...
  const accRes = await databases.listDocuments(DB_ID, COLLECTIONS.affiliateAccount, [
    QueryHelper.equal("userId", userId),
    QueryHelper.limit(1),
  ]);
  const account = (accRes?.documents || [])[0];
  if (!account) return { affiliateId: null, totalEarned: 0, referralsCount: 0, commissions: [] };
  ...
  const [refRes, comRes] = await Promise.all([
    ...
    databases.listDocuments(DB_ID, COLLECTIONS.affiliateCommissions, [
      QueryHelper.equal("affiliateId", String(affiliateId)), // your commissions affiliateId is string size 64
  ...
  return {
    ...
    referralsCount: (refRes?.documents || []).length,
