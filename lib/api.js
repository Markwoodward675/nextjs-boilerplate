...
// --- Appwrite session compat (works across SDK versions) ---
async function createEmailSessionCompat(account, email, password) {
  ...
  throw new Error(
    "Your Appwrite SDK does not support email sessions. Check that `lib/api.js` uses the **appwrite** package on the client (not node-appwrite), and update the SDK if needed."
  );
}
