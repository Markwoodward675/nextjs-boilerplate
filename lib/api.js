...
/* ===============================
   MISSING EXPORTS (transactions, alerts, profile)
=============================== */
...
/* ===============================
   VERIFY CODE via SERVER (Resend + Admin SDK)
   - send email: /api/auth/send-verify-code
...
export async function createOrRefreshVerifyCode(userId) {
  try {
    ...
    throw new Error(getErrorMessage(err, "Unable to send verification code email."));
  }
}
