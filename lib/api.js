...
// Optional but recommended (set on Vercel):
  ...
  try {
    ...
    if (!profile) {
      try {
        ...
        // Give a clearer error if the collection name is wrong/missing
...
export async function createOrRefreshVerifyCode(userId) {
  try {
    ...
    if (!res.ok || !json?.ok) {
      throw new Error(json?.error || "Unable to send verification code email.");
    }

    return true;
  } catch (err) {
    throw new Error(
      getErrorMessage(err, "Unable to send verification code email.")
    );
  }
}
...
export async function verifySixDigitCode(userId, code) {
  try {
    ...
    const res = await fetch("/api/auth/verify-code", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ userId: id, code: cleaned }),
    });
