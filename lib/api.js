...
// Flexible redirect URL for email verification
// 1) If you set NEXT_PUBLIC_APPWRITE_VERIFY_REDIRECT_URL in env, it uses that.
// 2) Otherwise, in the browser it uses `${window.location.origin}/verify`.
// 3) As a final fallback (during build), it uses a generic placeholder.
const VERIFIED_REDIRECT_URL =
  process.env.NEXT_PUBLIC_APPWRITE_VERIFY_REDIRECT_URL ||
  (typeof window !== "undefined"
    ? `${window.location.origin}/verify`
    : "https://day-traders-projects.vercel.app/verify");
...
async function createEmailSessionCompat(email, password) {
  ...
  if (typeof anyAccount.createEmailPasswordSession === "function") {
    // New SDK – can be called with object or positional args
    try {
    ...
    if (existingProfiles.total === 0) {
      await databases.createDocument(
        ...
        IDHelper.unique(),
        {
          ...
          avatarUrl: "", // kept but empty; can ignore on UI
          country: "",
          createdAt: now,
          updatedAt: now,
        }
 ...
 *
 * Behaviour:
 *  - If there's an existing session with the same email, reuse it.
 *  - If there's an existing session with a different email, delete it, then login.
 *  - Only show "Unable to reach Day Trader servers..." on true network errors.
 */
export async function loginWithEmailPassword(email, password) {
  try {
    ...
    try {
      existing = await account.get();
    } catch {
      existing = null;
    }

    // If already logged in as this email, just reuse
    if (existing && existing.email === email) {
      return existing;
    }

    // If logged in as a different email, clear that session first
    if (existing && existing.email !== email) {
      try {
        await account.deleteSession("current");
      } catch (e) {
        console.warn("Failed to delete existing session before login:", e);
      }
    }
...
// Get a single user profile document by userId
...
/**
 * Request a new 6-digit verification code.
 * Dev-mode: returns the code so you can show it on screen.
 * In real production you would email or SMS it instead.
 */
export async function requestVerificationCode() {
  ...
  await databases.updateDocument(DB_ID, COLLECTIONS.userProfiles, profile.$id, {
  ...
  // DEV MODE: return code so you can see it on the frontend
  return { code, expiresAt };
}

/**
 * Confirm a 6-digit verification code.
 */
export async function confirmVerificationCode(inputCode) {
  ...
  const current = await account.get().catch(() => null);
  if (!current) {
    throw new Error("You must be signed in to verify your code.");
  }
