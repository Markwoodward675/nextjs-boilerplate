...
export async function loginWithEmailPassword(email, password) {
  try {
    ...
    try {
      ...
      // It's fine if there was no active session
    ...
    try {
      ...
      // Network issues → show your branded Day Trader message
      if (
        typeof msg === "string" &&
      ...
      // Everything else (invalid credentials, etc.) bubble up as-is
      throw new Error(msg);
    }

    // 3) If session was created, return the fresh user
    const user = await account.get();
    return user;
  } catch (err) {
