...
async function ensureWallets(userId) {
  ...
  // IMPORTANT:
  // walletId must be <= 36 chars.
  // Appwrite's ID.unique() is usually longer, so we generate a 36-char UUID ourselves.
  const uuid36 = () => {
    if (typeof crypto !== "undefined" && crypto.randomUUID) return crypto.randomUUID(); // 36 chars
    // fallback if randomUUID isn't available (very rare in modern browsers)
  ...
  // NOTE: currencyType is enum in your schema.
  // Use ONLY values that exist in the enum options (check your Appwrite enum list).
  // Common choices: "main", "trading", "affiliate" OR "USD", etc.
  // I'm using "main", "trading", "affiliate" because you want 3 wallets.
  const makeWallet = (currencyType) => {
    ...
    return {
      walletId,
      userId,
      currencyType,   // enum: MUST match one of your allowed enum values
      balance: 0,
      isActive: true,
      createdDate: nowIso,
      updatedDate: nowIso,
  ...
  // Use docId = walletId (consistent + easy to fetch)
